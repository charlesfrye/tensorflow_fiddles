    ...
    with tf.variable_scope("crit_finder"):
        ...
        with tf.variable_scope("newton"):
            if "inverse_method" in hyperparameters.keys():
                inverse_hessian, parameter_placeholder = invert_hessian(hessian_matrix, input_size, hyperparameters)
                graph_dictionary["inverse_hessian"] = inverse_hessian
                graph_dictionary["inverse_parameter"] = parameter_placeholder
...
def invert_hessian(hessian, num_parameters, hyperparameters):
    ...
    if method == "fudged":
        parameter_placeholder = tf.placeholder(tf.float32, shape=[], name="fudge_factor")
        fudging_vector = tf.multiply(tf.ones(shape=num_parameters), parameter_placeholder, name="fudging_vector")
        ...
        parameter_placeholder = tf.placeholder(parameter_placeholder, shape=[], name="threshold")
        keep = tf.greater(tf.abs(eigenvalues), parameter_placeholder)
    ...
    return inverse_hessian, parameter_placeholder
